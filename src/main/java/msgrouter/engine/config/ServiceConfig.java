...
public class ServiceConfig {
	...
	public ServiceConfig(ServiceBootstrapConfig sbc, ClassLoader cl) throws TechException {
		this.sbc = sbc;
		this.cl = cl;

		if (sbc.getServiceType() < 0) {
			throw new TechException("Unknown service type: " + sbc.getServiceTypeName());
		}
		...
		try {
			...
			if (log.isInfoEnabled()) {
				log.info("parses node '" + sbc.getServiceTypeName() + "', " + ATTR_ID + "=" + sbc.getServiceId());
			}

			XmlReader xml = new XmlReader(new File(sbc.getServicePath() + File.separator + "service.xml"));
			Node elasticNode = xml.getNode("/service/elastic");

			String beanConfiguration = XmlUtil.getText(elasticNode, "beanConfiguration");
			if (beanConfiguration != null) {
				BeanMgr.getInstance().loadBeans(sbc.getServiceId(), xml.getDir(), beanConfiguration, cl);
			}
			...
			if (MsgRouter.getInstance().getSqlConnPoolManager() != null) {
				...
				if (sqlMapsNode != null) {
					sqlConnPoolName = XmlUtil.getAttributeStringValue(sqlMapsNode, "sqlConnPool");
					String sqlMapsPath = XmlUtil.getAttributeStringValue(sqlMapsNode, "xml");
					sqlMapsPath = FilePathUtil.getAbsolutePath(xml.getDir(), sqlMapsPath);
					SqlConnPool sqlConnPool = MsgRouter.getInstance().getSqlConnPoolManager()
							.getSqlConnPool(sqlConnPoolName);
					if (sqlConnPool == null) {
						throw new TechException(SqlConnPool.class.getSimpleName() + " '" + sqlConnPoolName
								+ "' is not defined at the " + SqlConnPoolManager.class.getSimpleName());
					}
					sqlConnPool.getSqlMaps().addAndInitSqlMapConfig(sqlMapsPath);
				}
			}
			...
			for (int i = 0; childs != null && i < childs.size(); i++) {
				...
				if (ThreadInitializer.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					String className = XmlUtil.getAttributeStringValue(svcChild, "class");
					thrInitClass = Class.forName(className, true, cl);
					if (!ReflectionUtil.isCastable(thrInitClass, ThreadInitializer.class)) {
						throw new TechException(className + " is not a " + ThreadInitializer.class.getName());
					}
				} else if (ServiceInitializer.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					String className = XmlUtil.getAttributeStringValue(svcChild, "class");
					svcInitClass = Class.forName(className, true, cl);
					if (!ReflectionUtil.isCastable(svcInitClass, ServiceInitializer.class)) {
						throw new TechException(className + " is not a " + ServiceInitializer.class.getName());
					}
				} else if (SessionInitializer.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					String className = XmlUtil.getAttributeStringValue(svcChild, "class");
					sessionInitClass = Class.forName(className, true, cl);
					if (!ReflectionUtil.isCastable(sessionInitClass, SessionInitializer.class)) {
						throw new TechException(className + " is not a " + SessionInitializer.class.getName());
					}
				} else if (SessionCloser.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					String className = XmlUtil.getAttributeStringValue(svcChild, "class");
					sessionCloserClass = Class.forName(className, true, cl);
					if (!ReflectionUtil.isCastable(sessionCloserClass, SessionCloser.class)) {
						throw new TechException(className + " is not a " + SessionCloser.class.getName());
					}
				} else if (AUTH_MANAGER.equalsIgnoreCase(svcChild.getName())) {
					String amConfPath = XmlUtil.getAttributeStringValue(svcChild, "xml");
					logDbPoolName = XmlUtil.getAttributeStringValue(svcChild, "logDbPoolName");
					amConfFullPath = FilePathUtil.getAbsolutePath(xml.getDir(), amConfPath);
					String userIdsStr = XmlUtil.getText(svcChild, SERVER_USERIDS);
					userIds = StringUtil.splitWithoutEmpty(userIdsStr, ",");
					if (log.isInfoEnabled()) {
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": users=" + userIdsStr);
					}
					String groupIdsStr = XmlUtil.getText(svcChild, SERVER_GROUPIDS);
					groupIds = StringUtil.splitWithoutEmpty(groupIdsStr, ",");
					if (log.isInfoEnabled()) {
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": groups=" + groupIdsStr);
					}
				} else if (X_ROUTING_TARGET.equalsIgnoreCase(svcChild.getName())) {
					String rtType = XmlUtil.getAttributeStringValue(svcChild, "type");
					...
					case Service.IVAL_ROUTING_TARGET_MSGROUTER_ID:
						dupAliveStr = XmlUtil.getAttributeStringValue(svcChild, SVAL_ROUTING_TARGET_MSGROUTER_ID,
								X_DUP_ALIVE);
						if (SVAL_DUP_ALIVE_FIRST.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_FIRST;
						} else if (SVAL_DUP_ALIVE_LAST.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_LAST;
						} else if (SVAL_DUP_ALIVE_ALL.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_ALL;
						}
						break;
					case Service.IVAL_ROUTING_TARGET_CUSTOM_ID:
						loginerClassName = XmlUtil.getAttributeStringValue(svcChild, SVAL_ROUTING_TARGET_CUSTOM_ID,
								"loginerClass");
						dupAliveStr = XmlUtil.getAttributeStringValue(svcChild, SVAL_ROUTING_TARGET_CUSTOM_ID,
								X_DUP_ALIVE);
						if (SVAL_DUP_ALIVE_FIRST.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_FIRST;
						} else if (SVAL_DUP_ALIVE_LAST.equalsIgnoreCase(dupAliveStr)) {
					...
					case Service.IVAL_ROUTING_TARGET_IP:
						dupAliveStr = XmlUtil.getAttributeStringValue(svcChild, SVAL_ROUTING_TARGET_IP, X_DUP_ALIVE);
						if (SVAL_DUP_ALIVE_FIRST.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_FIRST;
						} else if (SVAL_DUP_ALIVE_LAST.equalsIgnoreCase(dupAliveStr)) {
							dupAliveType = Service.IVAL_DUP_ALIVE_LAST;
					...
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": port=" + serverPort);

					minAcceptThrs = XmlUtil.getInt(svcChild, MIN_ACCEPT_THRS);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": minAcceptThrs="
								+ minAcceptThrs);

					minTransferThrs = XmlUtil.getInt(svcChild, MIN_TRANS_THRS);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": minTransferThrs="
								+ minTransferThrs);

					maxSessionsPerTransferThr = XmlUtil.getInt(svcChild, MAX_SESSIONS_PER_TRANS_THR);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": maxSessionsPerTransferThr="
								+ maxSessionsPerTransferThr);

					minBeanThrs = XmlUtil.getInt(svcChild, MIN_BEAN_THRS);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": minBeanThrs=" + minBeanThrs);

					maxSessionsPerBeanThr = XmlUtil.getInt(svcChild, MAX_SESSIONS_PER_BEAN_THR);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": maxSessionsPerBeanThr="
								+ maxSessionsPerBeanThr);

					maxSessions = XmlUtil.getInt(svcChild, MAX_SESSIONS);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": maxSessions=" + maxSessions);

					maxConnections = XmlUtil.getInt(svcChild, MAX_CONNECTIONS);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": maxConnections="
								+ maxConnections);
					...
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": dstPort=" + dstPort);

					dstIp = XmlUtil.getText(svcChild, CLIENT_DST_IP);
					if (log.isInfoEnabled())
						log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": dstIp=" + dstIp);

					dstUrlStr = XmlUtil.getText(svcChild, CLIENT_DST_URL);
					if (!StringUtil.isEmpty(dstUrlStr))
						if (log.isInfoEnabled())
							log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": dstUrl=" + dstUrlStr);

					reqEncoding = XmlUtil.getText(svcChild, CLIENT_REQ_ENCODING);
					if (!StringUtil.isEmpty(reqEncoding))
						if (log.isInfoEnabled())
							log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": reqEncoding="
									+ reqEncoding);
					...
					for (int j = 0; childList != null && j < childList.size(); j++) {
						...
						if ("props".equalsIgnoreCase(child.getName())) {
							...
							if (log.isDebugEnabled()) {
								log.debug(getServiceId() + "'s connectionProps=" + connectionProps);
							}
						} else if (MessageFactory.class.getSimpleName().equalsIgnoreCase(child.getName())) {
							String className = XmlUtil.getAttributeStringValue(child, "class");
							if (className != null && !"".equalsIgnoreCase(className)) {
								msgFactoryClass = Class.forName(className, true, cl);
								if (log.isInfoEnabled()) {
									log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": "
											+ MessageFactory.class.getSimpleName() + "=" + msgFactoryClass.getName());
								}
							}
						} else if (MessageRecver.class.getSimpleName().equalsIgnoreCase(child.getName())) {
							String className = XmlUtil.getAttributeStringValue(child, "class");
							if (className != null && !"".equalsIgnoreCase(className)) {
								recverClass = Class.forName(className, true, cl);
								if (log.isInfoEnabled()) {
									log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": "
											+ MessageRecver.class.getSimpleName() + "=" + recverClass.getName());
								}
							}
						} else if (MessageSender.class.getSimpleName().equalsIgnoreCase(child.getName())) {
							String className = XmlUtil.getAttributeStringValue(child, "class");
							if (className != null && !"".equals(className)) {
								senderClass = Class.forName(className, true, cl);
								if (log.isInfoEnabled()) {
									log.info(sbc.getServiceTypeName() + " " + sbc.getServiceId() + ": "
											+ MessageSender.class.getSimpleName() + "=" + senderClass.getName());
								}
							}
						}
					}
				} else if (Queue.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					this.qParams = parseQueueNode(svcChild);

				} else if (ProcessMap.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					String pmConfPath = XmlUtil.getAttributeStringValue(svcChild, "xml");
					/*
					 * ProcessMap configuration
					 */
					String pmConfFullPath = FilePathUtil.getAbsolutePath(xml.getDir(), pmConfPath);
					if (log.isInfoEnabled()) {
						log.info("loading " + pmConfFullPath);
					}
					this.procMap = new ProcessMap(this, pmConfFullPath, false, cl);
				} else if (MessageLogger.class.getSimpleName().equalsIgnoreCase(svcChild.getName())) {
					svcMsgLogParams = parseMessageLoggerNode(svcChild);
				}
			}

			QueueParams globalQP = MsgRouter.getInstance().getConfig().getMainConfig().getGlobalQueueParams();
			qParams.setUndefinedAttributes(globalQP);

			if (log.isInfoEnabled()) {
				log.info("Service " + getServiceId() + "'s " + QueueParams.class.getSimpleName() + "=" + qParams);
			}
	...
	private MessageLoggerParams parseMessageLoggerNode(Node msgLogNode) throws ClassNotFoundException {
		MessageLoggerParams params = new MessageLoggerParams();
		String logPerStr = XmlUtil.getAttributeStringValue(msgLogNode, "logPer");
		...
		params.setPath(XmlUtil.getText(msgLogNode, "root") + File.separator + "service-" + sbc.getServiceId(),
				"message.log");
	...
	private static String getAttributeStringValue(Node element, String[] attrNames) {
	...
	public static QueueParams parseQueueNode(Node queueNode) {
		...
		for (int i = 0; queueChilds != null && i < queueChilds.size(); i++) {
			...
			} else if ("inactiveTimeoutMinute".equalsIgnoreCase(argNode.getName())) {
	...
	public void setMessageFactoryClassName(String className) throws TechException {
		try {
			msgFactoryClass = Class.forName(className, true, cl);
	...
	public int getMinTransferThrs() {
		return intVal(minTransferThrs, 1);
	}
	...
	public int getMaxSessionsPerTransferThr() {
		return Config.IS_WINDOWS ? 63 : intVal(maxSessionsPerTransferThr, 1024);
	}
	...
	public int getMaxSessionsPerBeanThr() {
		return intVal(maxSessionsPerBeanThr, 1024);
	}

	public int getMaxConnections() {
		return intVal(maxConnections, Integer.MAX_VALUE);
	}

	public int getMaxSessions() {
		return intVal(maxSessions, getMaxConnections());
	}

	private static int intVal(Integer val, int defaultValue) {
		return (val != null && val.intValue() > 0) ? val.intValue() : defaultValue;
	}
	...
	public static final String xmlTag(String nodeName, Object value) {
		return "<" + nodeName + ">" + value + "</" + nodeName + ">" + CommonUtil.NEW_LINE;
	}

	public String toText() {
		StringBuilder sb = new StringBuilder();

		sb.append("		<" + sbc.getServiceTypeName() + " id=\"" + sbc.getServiceId() + "\">")
				.append(CommonUtil.NEW_LINE);
		sb.append("			<description>" + sbc.getDescription() + "</description>").append(CommonUtil.NEW_LINE);
		...
		if (userIds != null && userIds.length > 0) {
			sb.append("			" + xmlTag(SERVER_USERIDS, StringUtil.toCSVWithoutSpace(userIds)));
		}

		if (groupIds != null && groupIds.length > 0) {
			sb.append("			" + xmlTag(SERVER_GROUPIDS, StringUtil.toCSVWithoutSpace(groupIds)));
		}
		...
		if (recverClass != null) {
			...
			if (getConnectionProps() != null) {
				...
				if (getConnectionProps().get("encoding") != null) {
					sb.append("					<prop key=\"encoding\">" + getConnectionProps().get("encoding")
							+ "</prop>").append(CommonUtil.NEW_LINE);
				}

				if (getConnectionProps().get("recvTimeoutMillis") != null) {
					sb.append("					<prop key=\"recvTimeoutMillis\">"
							+ getConnectionProps().get("recvTimeoutMillis") + "</prop>").append(CommonUtil.NEW_LINE);
				}
				sb.append("				</props>").append(CommonUtil.NEW_LINE);
			}
			sb.append("				<receiver class=\"" + recverClass.getName() + "\" />").append(CommonUtil.NEW_LINE);
			sb.append("				<sender class=\"" + senderClass.getName() + "\" />").append(CommonUtil.NEW_LINE);
			sb.append("			</connection>").append(CommonUtil.NEW_LINE);
		}
		...
		if (maxSessionsPerTransferThr != null) {
			sb.append("			" + xmlTag(MAX_SESSIONS_PER_TRANS_THR, maxSessionsPerTransferThr));
		}
		if (minBeanThrs != null) {
			sb.append("			" + xmlTag(MIN_BEAN_THRS, minBeanThrs));
		}
		if (maxSessionsPerBeanThr != null) {
			sb.append("			" + xmlTag(MAX_SESSIONS_PER_BEAN_THR, maxSessionsPerBeanThr));
		}

		if (maxSessions != null) {
			sb.append("			" + xmlTag(MAX_SESSIONS, maxSessions));
		}

		if (maxConnections != null) {
			sb.append("			" + xmlTag(MAX_CONNECTIONS, maxConnections));
		}
	...
	public static String toConfigText(QueueParams qp) {
		...
		if (qp.getQueueType() == Queue.QTYPE_FILE) {
			if (qp.getBaseDir() != null) {
				sb.append("				<dir>" + qp.getBaseDir() + "</dir>").append(CommonUtil.NEW_LINE);
			}
			if (qp.getFileBlockEntries() != null) {
				sb.append("				<fileBlockEntries>" + qp.getFileBlockEntries() + "</fileBlockEntries>")
						.append(CommonUtil.NEW_LINE);
			}
			if (qp.isFileErrorFlexible() != null) {
				sb.append("				<fileErrorFlexible>" + qp.isFileErrorFlexible() + "</fileErrorFlexible>")
						.append(CommonUtil.NEW_LINE);
			}
			if (qp.getInactiveTimeoutMinute() != null) {
				sb.append("				<inactiveTimeoutMinute>" + qp.getInactiveTimeoutMinute()
						+ "</inactiveTimeoutMinute>").append(CommonUtil.NEW_LINE);
			}
			if (qp.isPersistent() != null) {
				sb.append("				<persistent>" + qp.isPersistent() + "</persistent>")
						.append(CommonUtil.NEW_LINE);
			}
		} else if (qp.getQueueType() == Queue.QTYPE_MEMORY) {
		}
		...
		if (qp.getMemoryLoadEntries() != null) {
			sb.append("				<memoryLoadEntries>" + qp.getMemoryLoadEntries() + "</memoryLoadEntries>")
					.append(CommonUtil.NEW_LINE);
		}

		if (qp.getTimeoutMillisecond() != null) {
			sb.append("				<timeoutMillisecond>" + qp.getTimeoutMillisecond() + "</timeoutMillisecond>")
					.append(CommonUtil.NEW_LINE);
		}
		sb.append("			</queue>");
		return sb.toString();
